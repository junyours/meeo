...
public function rentalReport(Request $request)
{
    ...
    // Group by section first, then by vendor within each section
    $reportData = [];
    
    foreach ($rentals as $rental) {
        $vendorName = $rental->vendor->fullname ?? 'Unknown Vendor';
        $section = $rental->stall->section;
        $area = $section->area ?? null;
        $stallNumber = $rental->stall->stall_number ?? 'N/A';
        $dailyRent = floatval($rental->daily_rent ?? 0);
        $monthlyRent = floatval($rental->monthly_rent ?? 0);
        
        // Determine section name - if area is "Open Space" or similar, use "Open Space"
        $sectionName = 'Open Space';
        if ($area && strtolower(trim($area->name)) !== 'open space') {
            $sectionName = $section->name ?? 'Unknown Section';
        }
        
        // Create unique key for section-vendor combination
        $sectionVendorKey = $sectionName . '|' . $vendorName;
        
        if (!isset($reportData[$sectionVendorKey])) {
            $reportData[$sectionVendorKey] = [
                'section_name' => $sectionName,
                'vendor_name' => $vendorName,
                'stall_numbers' => [],
                'daily_rental_total' => 0,
                'monthly_rental_total' => 0
            ];
        }
        
        // Add stall number if not already present
        if (!in_array($stallNumber, $reportData[$sectionVendorKey]['stall_numbers'])) {
            $reportData[$sectionVendorKey]['stall_numbers'][] = $stallNumber;
        }
        
        // Add to daily and monthly totals
        $reportData[$sectionVendorKey]['daily_rental_total'] += $dailyRent;
        $reportData[$sectionVendorKey]['monthly_rental_total'] += $monthlyRent;
    }
    
    // Convert to indexed array and sort by section name, then vendor name
    $finalReport = collect(array_values($reportData))->sortBy(function($item) {
        return $item['section_name'] . '|' . $item['vendor_name'];
    })->values()->toArray();
    
    // Calculate totals
    $totalDailyRental = collect($finalReport)->sum('daily_rental_total');
    $totalMonthlyRental = collect($finalReport)->sum('monthly_rental_total');
    
    return response()->json([
        'status' => 'success',
        'data' => $finalReport,
        'totals' => [
            'daily_rental' => $totalDailyRental,
            'monthly_rental' => $totalMonthlyRental
        ]
    ]);
}
}
